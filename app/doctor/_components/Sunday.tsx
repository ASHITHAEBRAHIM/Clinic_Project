...
type Props = {
    upsertAvailability: (day: 'sunday', times: string[]) => Promise<any>;
};

const Sunday: React.FC<Props> = ({ upsertAvailability }) => {
        ...
        const fetchUserIdAndAvailability = async () => {
            try {
                ...
                if (userData && userData.user) {
                    ...
                    if (data) {
                        const storedTimes = data.sunday.map((utcTime: string) => {
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            if (existingData) {
                await upsertAvailability('sunday', selectedTimesUTC);
            } else {
                await upsertAvailability('sunday', selectedTimesUTC);
            }
