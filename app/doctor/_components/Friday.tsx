...
type Props = {
    upsertAvailability: (day: 'friday', times: string[]) => Promise<any>;
};

const Friday: React.FC<Props> = ({ upsertAvailability }) => {
        ...
        const fetchUserIdAndAvailability = async () => {
            try {
                ...
                if (userData && userData.user) {
                    ...
                    if (data) {
                        const storedTimes = data.friday.map((utcTime: string) => {
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            if (existingData) {
                await upsertAvailability('friday', selectedTimesUTC);
            } else {
                await upsertAvailability('friday', selectedTimesUTC);
            }
