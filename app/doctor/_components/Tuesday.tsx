...
type Props = {
    upsertAvailability: (day: 'tuesday', times: string[]) => Promise<any>;
};

const Tuesday: React.FC<Props> = ({ upsertAvailability }) => {
        ...
        const fetchUserIdAndAvailability = async () => {
            try {
                ...
                if (userData && userData.user) {
                    ...
                    if (data) {
                        const storedTimes = data.tuesday.map((utcTime: string) => {
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            if (existingData) {
                await upsertAvailability('tuesday', selectedTimesUTC);
            } else {
                await upsertAvailability('tuesday', selectedTimesUTC);
            }
