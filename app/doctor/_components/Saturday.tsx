...
type Props = {
    upsertAvailability: (day: 'saturday', times: string[]) => Promise<any>;
};

const Saturday: React.FC<Props> = ({ upsertAvailability }) => {
        ...
        const fetchUserIdAndAvailability = async () => {
            try {
                ...
                if (userData && userData.user) {
                    ...
                    if (data) {
                        const storedTimes = data.saturday.map((utcTime: string) => {
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            if (existingData) {
                await upsertAvailability('saturday', selectedTimesUTC);
            } else {
                await upsertAvailability('saturday', selectedTimesUTC);
            }
