...
type Props = {
    upsertAvailability: (day: 'thursday', times: string[]) => Promise<any>;
};

const Thursday: React.FC<Props> = ({ upsertAvailability }) => {
        ...
        const fetchUserIdAndAvailability = async () => {
            try {
                ...
                if (userData && userData.user) {
                    ...
                    if (data) {
                        const storedTimes = data.thursday.map((utcTime: string) => {
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            if (existingData) {
                await upsertAvailability('thursday', selectedTimesUTC);
            } else {
                await upsertAvailability('thursday', selectedTimesUTC);
            }
