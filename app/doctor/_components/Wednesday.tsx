...
type Props = {
    upsertAvailability: (day: 'wednesday', times: string[]) => Promise<any>;
};

const Wednesday: React.FC<Props> = ({ upsertAvailability }) => {
        ...
        const fetchUserIdAndAvailability = async () => {
            try {
                ...
                if (userData && userData.user) {
                    ...
                    if (data) {
                        const storedTimes = data.wednesday.map((utcTime: string) => {
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            const selectedTimesUTC = selectedTimes.map(time => {
                ...
                const utcTime = moment.utc(localTime).format(); // Convert to string
                return utcTime;
            });
            ...
            if (existingData) {
                await upsertAvailability('wednesday', selectedTimesUTC);
            } else {
                await upsertAvailability('wednesday', selectedTimesUTC);
            }
